...
parser.add_argument("interval", nargs="?", default=5, type=range_check,
...
while i != args.count and not exiting:
    ...
    for k, (send_bytes, recv_bytes) in sorted(ipv4_throughput.items(),
        ...
        recv_by += int(recv_bytes )
        send_by += int(send_bytes )
    ...
    def get_meminfo():
        with open('/proc/meminfo', 'r') as file_stat:
            mem_lines = []
            for lines in file_stat.readlines()[0:3]:
                mem_lines.append(lines.replace(" ", "").replace("kB\n", "").replace(":", " = "))
    ...
    def get_cpu_usage_percentage():
        ...
        for cpu in start:
            diff_total = start[cpu]['total']
            diff_idle = start[cpu]['idle']
            cpu_usage_percentage = ((diff_total - diff_idle) / diff_total) * 100

        return cpu_usage_percentage

    print("user = %s, cpu = %s, %s, %s, %s, recv_packet = %d, send_packet = %d" %
          (os.environ.get('HOME'),get_cpu_usage_percentage(), get_meminfo()[0], get_meminfo()[1], get_meminfo()[2], recv_by, send_by))

    if get_cpu_usage_percentage() >= 50 : print("cpu 사용량이 너무 높습니다.")

    msg_total = save_print_instance("user = %s, cpu = %s, %s, %s, %s, recv_packet = %d, send_packet = %d" %
          (os.environ.get('HOME'),get_cpu_usage_percentage(), get_meminfo()[0], get_meminfo()[1], get_meminfo()[2], recv_by, send_by))
    sock.sendto(msg_total.encode(), ("10.9.136.197", 8888))
    i += 1
